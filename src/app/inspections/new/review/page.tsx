...
function NewInspectionReviewContent() {
  ...
  const inspectionType = inspectionData.inspectionTypeCategories
    ...
    .find(type => slugify(type) === inspectionTypeParam);
                ...
                <div className="grid gap-4">
                    <div className="flex items-center justify-between">
                        <h3 className="font-semibold text-lg">Property & Client</h3>
                         <Button variant="ghost" size="sm" asChild>
                            <Link href={{pathname: `/inspections/new/${inspectionTypeParam}`, query: {clientId: clientId ?? undefined}}}><Edit className="mr-2 h-4 w-4" />Edit</Link>
                        </Button>
                    </div>
