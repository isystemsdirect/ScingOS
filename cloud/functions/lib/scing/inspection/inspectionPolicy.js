...
exports.evaluateFinalize = evaluateFinalize;
function evaluateFinalize(params) {
    const reasons = [];
    ...
    if (failed.length > 0)
        reasons.push('ARTIFACT_INTEGRITY_FAILED');
    const notFinalized = artifacts.filter((a) => !a.finalized);
    if (notFinalized.length > 0)
        reasons.push('ARTIFACTS_NOT_FINALIZED');
    // type policy
    ...
    if (requiredTypes.size > 0) {
        for (const t of requiredTypes) {
            ...
            if (!has)
                reasons.push(`MISSING_REQUIRED_TYPE:${t}`);
        }
    }
    ...
    if (!online) {
        ...
        const denyStatus = inspection.status === 'archived'
            ? 'archived'
...
//# sourceMappingURL=inspectionPolicy.js.map