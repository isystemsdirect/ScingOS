...
const vectorsPath = path.join(repoRoot, 'scing', 'weather', 'tests', 'severity.test.ts');
const { SEVERITY_TEST_VECTORS, runSeverityVectors } = loadTsModule(vectorsPath);

assert.ok(Array.isArray(SEVERITY_TEST_VECTORS), 'SEVERITY_TEST_VECTORS must be an array');
runSeverityVectors();

console.log(`weather severity selftest: OK (${SEVERITY_TEST_VECTORS.length} vectors)`);
